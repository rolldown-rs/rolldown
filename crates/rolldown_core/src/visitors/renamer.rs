use ast::{Id, Ident, ExportNamedSpecifier};
...
pub struct Renamer<'a> {
    pub uf: &'a mut UFriend<Id>,
    pub rename_map: &'a HashMap<Id, JsWord>,
}

impl<'a> VisitMut for Renamer<'a> {
    fn visit_mut_ident(&mut self, ident: &mut Ident) {
        if let Some(root_id) = self.uf.find_root(&ident.to_id()) {
    ...
    fn visit_mut_export_named_specifier(&mut self, node: &mut ExportNamedSpecifier) {
        ...
        let is_eq = node.exported.as_ref().and_then(|module_export_name| {
